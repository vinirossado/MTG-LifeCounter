...
struct CommanderDetailView: View {
    ...
    init(commander: ScryfallCard, onSelect: @escaping (ScryfallCard) -> Void, onSelectAsBackground: ((ScryfallCard) -> Void)? = nil) {
                ...
                if let imageURL = commander.imageURL {
                    AsyncImage(url: URL(string: imageURL)) { phase in
                        ...
                        case .failure(let error):
                            ...
                            .frame(height: 300)
                            .onAppear {
                                print("❌ Commander detail image failed: \(imageURL) - Error: \(error.localizedDescription)")
                            }
                        case .empty:
                            VStack(spacing: 16) {
                                ProgressView()
